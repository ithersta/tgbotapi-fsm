...
typealias AppliedHandler<BS> = suspend (RequestsExecutor, suspend (BS) -> Unit, suspend () -> Unit) -> Unit
...
class Trigger<BS : Any, BU : Any, S : BS, U : BU, D>(
    ...
    fun handler(update: Update, state: S, user: U): AppliedHandler<BS>? {
        ...
        return { requestsExecutor, setState, refreshCommands ->
            StatefulContext<BS, BU, S, U>(requestsExecutor, state, setState, refreshCommands, update, user).handler(data)
        }
    }
}
